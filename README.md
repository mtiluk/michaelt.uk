...
All content for the site is done using markdown or JSON - there is no database or CMS to keep the site as simple as possible. 
```
content/
...
Footnote-style references at the bottom of a post are parsed out of the body.
...
components/
├── article/    long-form rendering: MDX components, references, share
...
Serves on port 3001, expecting a reverse proxy in front for TLS.
